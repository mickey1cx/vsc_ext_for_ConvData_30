...
Процедура РедактированиеПравил(ОбъектСсылка, Форма) Экспорт
	...
	Если ЗначениеЗаполнено(ПараметрыОбработчика) Тогда
		ЗапускРедактора(ОбъектФормы, ПараметрыОбработчика);
	Иначе
...
Процедура ЗапускРедактора(ОбъектФормы, ПараметрыОбработчика)
	
	ФайлОбработчика = ФайлОбработчика(ПараметрыОбработчика);
	ДополнительныеПараметры = Новый Структура("ФайлОбработчика, ОбъектФормы, РеквизитКода", ФайлОбработчика, ОбъектФормы, ПараметрыОбработчика.РеквизитКода);
...
Процедура ЗапускРедактора_Завершение(КодВозврата, ДополнительныеПараметры) Экспорт
	...
	Если НЕ КодВозврата = Неопределено Тогда
		ДополнительныеПараметры.ОбъектФормы[ДополнительныеПараметры.РеквизитКода] = ПолучитьТекстОбработчика(ФайлОбработчика);
	КонецЕсли;
