...
Процедура ЗапускРедактора(ОбъектСсылка, Форма) Экспорт
	...
	Иначе
		ВызватьИсключение "Обработчик не описан.";
	КонецЕсли;
	
	ФайлОбработчика = ФайлОбработчика(ПараметрыОбработчика);
	ЗапуститьПриложение(ФайлОбработчика,, Истина);
	ОбъектФормы[ПараметрыОбработчика.РеквизитКода] = ПолучитьТекстОбработчика(ФайлОбработчика);
