...
Процедура vsc_СоздатьФайлОбработкиМенеджераОбмена(Команда)
	...
	КаталогДляВременныхФайлов 	= "c:\Les\-=ОбменДанными=-\МодулиМенеджераОбмена\ХХХХХХ\";
	
	// Проверям если каклог не существует, то создаем его.	
	ВыбФайл = Новый Файл(КаталогДляВременныхФайлов);		
	Если Не ВыбФайл.Существует() Тогда
		СоздатьКаталог(КаталогДляВременныхФайлов);
	КонецЕсли;	
	
	ВыбФайл = Новый Файл(КаталогДляГотовыхФайлов);		
	Если Не ВыбФайл.Существует() Тогда
		СоздатьКаталог(КаталогДляГотовыхФайлов);
	КонецЕсли;	
	
	// Создаем внешнюю обработку в формате xml.
	// Сохраняем макет внешней обработки.
	...
	СоздатьКаталог(КаталогДляВременныхФайлов + "Обработка\Ext\");
	...
	ТекДок.Записать(КаталогДляВременныхФайлов+"Обработка\Ext\ObjectModule.bsl"); 
	
	// Выполняем загрузку внешней обработки (отчета) из формата XML.
	
	ВременныйКаталогБазы	= "C:\Les\-=ОбменДанными=-\КД";
	ВременныйКаталогБазы	= "E:\АльфоАвто обмен данными\tolik\-=Авто=-\КД Пустая";
	
	ВерсияПлатформыДляГенерацииEPF = "C:\Program Files (x86)\1cv8\8.3.12.1469\bin\";
	//ИмяВременногоEPF = КаталогДляГотовыхФайлов + "ХХХХ.epf";
	
	ИмяВременногоEPF = КаталогДляГотовыхФайлов + ИмяВнешнейОбработки();// + ".epf";
...
Функция ИмяВнешнейОбработки()
	Возврат "МенеджерОбмена" + СокрЛП(ОбщегоНазначения.ЗначениеРеквизитаОбъекта(Объект.Конвертация, "Наименование"));	
КонецФункции	
